...
// -----------------------------------------------------------------------------
    ...
    wire  [2:0] addr_in_port;   // addr_opess input port
...
// -----------------------------------------------------------------------------
  ...
  assign active_op2 = (addr_in_port == 3'b010) & !no_port;

  // addr_opess/control output decode
  assign i_hselm     = (addr_in_port == 3'b010) & sel_op2 & !no_port;
  assign HADDRM      = ( (addr_in_port == 3'b010) & !no_port ) ? addr_op2 : {32{1'b0}};
  assign HAUSERM     = ( (addr_in_port == 3'b010) & !no_port ) ? auser_op2 : {32{1'b0}};
  assign i_htransm   = ( (addr_in_port == 3'b010) & !no_port ) ? trans_op2 : 2'b00;
  assign HWRITEM     = (addr_in_port == 3'b010) & write_op2 & !no_port;
  assign HSIZEM      = ( (addr_in_port == 3'b010) & !no_port ) ? size_op2 : 3'b000;
  assign i_hburstm    = ( (addr_in_port == 3'b010) & !no_port ) ? burst_op2 : 3'b000;
  assign HPROTM      = ( (addr_in_port == 3'b010) & !no_port ) ? prot_op2  : {4{1'b0}};
  assign HMASTERM    = ( (addr_in_port == 3'b010) & !no_port ) ? master_op2 : 4'b0000;
  assign i_hmastlockm = (addr_in_port == 3'b010) & mastlock_op2 & !no_port;
  ...
  assign HWDATAM = ( data_in_port == 3'b010 ) ? wdata_op2 : {32{1'b0}};

  // HWUSERM output decode
  assign HWUSERM = ( data_in_port == 3'b010 ) ? wuser_op2 : {32{1'b0}};
