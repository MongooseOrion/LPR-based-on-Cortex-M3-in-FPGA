...
// Signal declarations
// -----------------------------------------------------------------------------
    wire [2:0] addr_in_port_next; // D-input of addr_in_port
...
//  input port should be deselected.
  ...
  assign request = req_port2 | ( (iaddr_in_port == 3'b010) & HSELM &
                                (HTRANSM != 2'b00) );

  assign no_port_next     = ! ( HMASTLOCKM | request | HSELM );
  assign addr_in_port_next = ( request & !HMASTLOCKM ) ? 3'b010 : iaddr_in_port;
